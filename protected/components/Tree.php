...
//     public $icon = array('│','├','└');  
    ...
    */  
     ...
     * @param unknown $str 生成树型结构的基本代码，例如："<option value=\$id \$selected>\$spacer\$name</option>" 
     * @param number $sid 被选中的ID，比如在做树型下拉框的时候需要用到 
     * @param string $adds
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = ''){  
        ...
        if(is_array($child)){  
            ...
            foreach($child as $id=>$value){  
                ...
                $name = '用户'.$value['id'];
                $detail = '联系人'.$value['contact'].'；联系方式'.$value['phone'];
                $spacer = $adds ? $adds.$j : '';  
                $selected = $id==$sid ? 'selected' : '';  
                @extract($value);  
                ($value['parentid'] == 0 && $str_group) ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");  
                $this->ret .= $nstr;  
                $nbsp = $this->nbsp;  
